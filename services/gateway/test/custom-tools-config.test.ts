...
describe('configService.getCustomToolMode', () => {
  it('resolves the mode shipped in api_config.json', () => {
    expect(configService.getCustomToolMode()).toBe('translate');
  });

  it('is reachable off the default export, which is how the plugin calls it', () => {
    expect(typeof configService.getCustomToolMode).toBe('function');
  });
});
