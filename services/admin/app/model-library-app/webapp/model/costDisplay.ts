...
export interface PriceRow { source: string; inputCost: string | null; outputCost: string | null; cacheReadInputCost: string | null; cacheCreationInputCost: string | null; }
export interface CostRow { label: string; key: 'input' | 'output' | 'cacheRead' | 'cacheCreation'; value: string | null; bracket: string; factor: string | null; }

// Labels match SAP AI Launchpad's Cost tab ("Input Token Cost Factor", ...).
const DIRECTIONS: { key: CostRow['key']; label: string; sap: keyof SnapshotCosts; price: keyof PriceRow }[] = [
