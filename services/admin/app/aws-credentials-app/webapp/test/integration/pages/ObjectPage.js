sap.ui.define(["sap/fe/test/ObjectPage", "sap/ui/test/Opa5"], function (ObjectPage, Opa5) {
	...
	return new ObjectPage({ appId: "admin.awscredentials", componentId: "AwsCredentialsObjectPage", entitySet: "AwsCredentials" }, {
		assertions: {
			...
			iSeeExpiresAtAboutDaysAhead: function (iDays) {
				return this.waitFor({
					controlType: "sap.uxap.ObjectPageLayout",
					check: function (aLayouts) {
						return typeof readStored(aLayouts[0], "expiresAt") === "string";
					},
					success: function (aLayouts) {
						var iActual = (new Date(readStored(aLayouts[0], "expiresAt")).getTime() - Date.now()) / DAY_MS;
						Opa5.assert.ok(Math.abs(iActual - iDays) <= 1,
							"Expires At is " + iActual.toFixed(2) + " days ahead (expected about " + iDays + ")");
					},
					errorMessage: "Expires At never got a value"
				});
			},
