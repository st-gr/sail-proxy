-- In-place migration for an EXISTING local SQLite admin database (image output pricing, 2026-09-15).
...
--   cp db/admin.db db/admin.db.before-image-output
-- Apply with:
--   sqlite3 db/admin.db < migrate-image-output.sqlite.sql
-- The statements are the compiler's own (npx cds compile src/db src/srv --to sql --dialect sqlite),
-- reduced to the delta: two new columns on ApiKeyUsage and AwsCredentialUsage (imageOutputTokens,
-- imageOutputCost), one new column on ModelCosts (imageOutputCost), the same two columns on
-- AwsCredentialUsage's draft table, and the six views that select the changed columns.
BEGIN;
...
ALTER TABLE sap_llm_gateway_admin_ApiKeyUsage ADD COLUMN imageOutputTokens INTEGER;
ALTER TABLE sap_llm_gateway_admin_ApiKeyUsage ADD COLUMN imageOutputCost DECIMAL(10, 6);

ALTER TABLE sap_llm_gateway_admin_AwsCredentialUsage ADD COLUMN imageOutputTokens INTEGER;
ALTER TABLE sap_llm_gateway_admin_AwsCredentialUsage ADD COLUMN imageOutputCost DECIMAL(10, 6);

ALTER TABLE sap_llm_gateway_admin_ModelCosts ADD COLUMN imageOutputCost DECIMAL(10, 6);

ALTER TABLE AdminService_AwsCredentialUsage_drafts ADD COLUMN imageOutputTokens INTEGER NULL;
ALTER TABLE AdminService_AwsCredentialUsage_drafts ADD COLUMN imageOutputCost DECIMAL(10, 6) NULL;
...
-- ModelPrices projection gained imageOutputCost (compiled definition).
