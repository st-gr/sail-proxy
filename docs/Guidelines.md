...
This repository is organized as a monorepo using pnpm workspaces. The main directories are:

- **[./services/](../services/)** - Contains the microservices:
  - `gateway` - The main Gateway service for routing and provider integration
  - `admin` - The Admin service for API key and credential management
  - `ollama` - The Ollama service for local model integration
- **[./libs/](../libs/)** - Shared libraries and utilities:
  - `auth` - Authentication and authorization utilities
  - `logger` - Shared logging utilities
  - `test-utils` - Common testing utilities and factories
  - `types` - Shared TypeScript type definitions
- **[./npm-dist/](../npm-dist/)** - Distribution packages for npm publishing

Each service and library is a separate package with its own `package.json`. The workspace configuration is defined in `pnpm-workspace.yaml` at the root of the repository.
...
We **mandate** the use of [Conventional Commits](https://www.conventionalcommits.org/en/v1.0.0/) specification for all commits in this repository. This is required to enable automated release management using [release-please](https://github.com/googleapis/release-please).

### Why Conventional Commits?

Conventional Commits provides a structured commit message format that enables:
- **Automated version management** - release-please automatically determines version bumps based on commit types
- **Automated CHANGELOG generation** - release notes are generated from commit messages
- **Clear commit history** - standardized format makes the git history easy to navigate
- **Automated releases** - GitHub Actions can automatically create releases based on commits

### Enforcement

All pull requests must follow Conventional Commits format. Non-compliant commits will be rejected during the PR review process.
